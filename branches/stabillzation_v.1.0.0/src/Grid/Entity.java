/**
 * The abstract Entity class is derived from SimBaseEntity and is used as a base
 * starting point for all grid nodes. It is basically a network-aware version of
 * SimBaseEntity. It keeps track of the wavelengts used by this enitity.
 *
 * @since version 2.0
 *
 * This entity also follows the MVC design pattern. We can plug in different
 * listeners {@link ChangeListener} which listen to events generated by this
 * entity. This is mainly for personal stat checking and updates.
 *
 * @version 2.0
 */
package Grid;
...
public abstract class Entity extends SimBaseEntityImpl {
    ...
    public double getFreeBandwidth(GridOutPort port, int lambda, Time t) {
    ...
    /**
     * Finds a wavelength that is free, given an outport (link) to take. The way
     * it is chosen is at random and when only one wavelength exist 0 is chosen
     * as the wavelength. If all wavelengths are occupied, the return value is
     * -1. This method is used for the send routines in the network, NOT to find
     * a free wavelength to set up a OCS-circuit.
     *
     * @param port The GridOutPort on which the message needs to be send.
     * @return A positive integer (wavelength), or -1 if all wavelengths are
     * occupied.
     */
     ...
     *
     * @param destination the nextHop node inside the grid
     ...
     * @param permanent Is it setup on the fly by the entity (is the enitity
     * also responsible for the tear down) or should the circuit be permanent.
    ...
    /**
     * Will find the ouport to go to nextHop. If there are more routes to the
     * nextHop then the first encoutered will be returned.
     *
     * @param nextHop The nextHop to be reached by this outport.
     * @return The first outport pointing to the nextHop, or null if there is
     * none.
     */
    ...
    /**
     * Will find the outport to the given nextHop using the wavelength for this
     * the circuit (if OCS is used). If multiple routes exist to nextHop the
     * first encoutered shall be returned
     *
